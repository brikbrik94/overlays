# Sprite assets

Lege hier die Sprite-Quellen für `rd`, `nah`, `nef` und `brd` ab
(z. B. `*.png` und ggf. generierte `sprite.json`/`sprite.png` Varianten).

Empfohlene Quellstruktur:
- `assets/sprites/rd/*.png`
- `assets/sprites/nah/*.png`
- `assets/sprites/nef/*.png`
- `assets/sprites/brd/*.png`

Sprite-Build ausführen:
```bash
python3 scripts/build_sprites.py --source assets/sprites --out dist/assets/sprites
```

Dieser Ordner ist die zentrale Quelle für die spätere Bereitstellung über
`/assets/sprites/...` im Hosted-Setup.


SVG-Konvertierung (SVG -> PNG):
```bash
python3 scripts/convert_sprite_svgs.py --source assets/sprites --out assets/sprites/png
# oder ohne Aktivierung direkt über die venv:
.venv/bin/python scripts/convert_sprite_svgs.py --source assets/sprites --out assets/sprites/png
```

Danach Sprite-Build ausführen:
```bash
python3 scripts/build_sprites.py --source assets/sprites/png --out dist/assets/sprites
```

Python-Abhängigkeiten (Debian 13 kompatibel via venv):
```bash
bash scripts/install_python_deps.sh
```

Extraktion aus einem großen Vorschau-SVG (Text entfernen, Metadaten erhalten):
```bash
python3 scripts/extract_sprite_icons.py --input assets/sprites/<dein-preview>.svg --out assets/sprites/extracted
```

Danach Konvertierung + Sprite-Build:
```bash
.venv/bin/python scripts/convert_sprite_svgs.py --source assets/sprites/extracted --out assets/sprites/png
python3 scripts/build_sprites.py --source assets/sprites/png --out dist/assets/sprites
```

Ein-Kommando-Pipeline (venv + Extraktion + SVG->PNG + Sprite-Build):
```bash
bash scripts/run_sprite_pipeline.sh
```

Optional nur eine Datei:
```bash
bash scripts/run_sprite_pipeline.sh --input assets/sprites/<dein-preview>.svg
```

Ergebnisstruktur in `dist/assets/sprites/`:
- `rd/sprite(.json|.png)`
- `nef/sprite(.json|.png)`
- `nah/sprite(.json|.png)`
- `brd/sprite(.json|.png)`
- `fallback/sprite(.json|.png)`
